...
public class MetadataTest extends AbstractTest {
  ...
  @Test
  public void demo() {
    final XMLMetadata metadata = client.getDeserializer(ContentType.APPLICATION_XML).toMetadata(getClass()
        .getResourceAsStream("demo-metadata.xml"));
    ...
    final Edm edm = client.getReader().readMetadata(getClass().getResourceAsStream("demo-metadata.xml"));
    assertNotNull(edm);
...
//    assertTrue(EdmAnnotationsTarget.TargetType.Property == annotationsTarget.getAnnotationsTargetType());
  ...
  public void getContainerWithoutCallingGetSchemas() {
    final XMLMetadata metadata = client.getDeserializer(ContentType.APPLICATION_XML).toMetadata(getClass()
        .getResourceAsStream("fromdoc1-metadata.xml"));
  ...
  public void fromdoc1() {
    ...
    final CsdlFunctionImport functionImport = metadata.getSchema(0).getEntityContainer().getFunctionImport(
        "ProductsByRating");
  ...
  public void fromdoc2() {
    ...
    // Check Tags
    final CsdlAnnotation tags = metadata.getSchema(0).getAnnotationGroup("ODataDemo.Product", null).getAnnotation(
        "Vocabulary1.Tags");
    assertNotNull(tags);
  ...
  public void fromdoc4() {
    final XMLMetadata metadata = client.getDeserializer(ContentType.APPLICATION_XML).toMetadata(getClass()
        .getResourceAsStream("fromdoc4-metadata.xml"));
    assertNotNull(metadata);
  ...
  public void issueOLINGO1232() {
    XMLMetadata xmlMetadata = client.getDeserializer(ContentType.APPLICATION_XML).toMetadata(getClass()
        .getResourceAsStream("caps.products.CatalogService_default.xml"));
    assertNotNull(xmlMetadata);
    assertEquals(94, xmlMetadata.getSchema(0).getAnnotationGroups().size());
    List<CsdlExpression> expressions = xmlMetadata.getSchema(0).getAnnotationGroups().get(0).getAnnotation(
        "UI.LineItem").getExpression().asDynamic().asCollection().getItems();
  ...
  public void readAnnotationOnAnEntityType() {
    ...
    assertEquals("ThingPerspective", navProperty.getAnnotations().get(0).getTerm().getName());
  }
  ...
  public void readAnnotationOnAProperty() {
    ...
    assertEquals("UI.HeaderInfo", property.getAnnotations().get(0).getTerm().getFullQualifiedName()
  ...
  public void readAnnotationWithinMetadataFile() {
    ...
    assertNotNull(annotation);
    valueAsString = annotation.getExpression().asConstant().getValueAsString();
    assertEquals("SAP HANA" , valueAsString);
